...
server <- function(input, output, session) {
  ...
  observeEvent(input$select_geography_o1, {
    selected_geography(input$select_geography_o1)
  })

  observeEvent(input$select_geography_o2, {
    selected_geography(input$select_geography_o2)
  })

  observeEvent(input$select_geography_o3, {
    selected_geography(input$select_geography_o3)
  })

  observeEvent(input$select_geography_o4, {
    selected_geography(input$select_geography_o4)
  })

  observeEvent(input$select_geography_e2, {
    selected_geography(input$select_geography_e2)
  })

  observeEvent(input$select_geography_e3, {
    selected_geography(input$select_geography_e3)
  })

  observe({
    current_geography <- selected_geography()
    updateVarSelectizeInput(session, "select_geography_o1", selected = current_geography)
    updateVarSelectizeInput(session, "select_geography_o2", selected = current_geography)
    updateVarSelectizeInput(session, "select_geography_o3", selected = current_geography)
    updateVarSelectizeInput(session, "select_geography_o4", selected = current_geography)
    updateVarSelectizeInput(session, "select_geography_e2", selected = current_geography)
    updateVarSelectizeInput(session, "select_geography_e3", selected = current_geography)
  })

  selected_breakdown <- reactiveVal(NULL)

  observeEvent(input$geographic_breakdown_o1, {
    selected_breakdown(input$geographic_breakdown_o1)
  })

  observeEvent(input$geographic_breakdown_o2, {
    selected_breakdown(input$geographic_breakdown_o2)
  })

  observeEvent(input$geographic_breakdown_o3, {
    selected_breakdown(input$geographic_breakdown_o3)
  })

  observeEvent(input$geographic_breakdown_o4, {
    selected_breakdown(input$geographic_breakdown_o4)
  })

  observeEvent(input$geographic_breakdown_e2, {
    selected_breakdown(input$geographic_breakdown_e2)
  })

  observeEvent(input$geographic_breakdown_e3, {
    selected_breakdown(input$geographic_breakdown_e3)
  })

  observe({
    current_breakdown <- selected_breakdown()
    updateVarSelectizeInput(session, "geographic_breakdown_o1", selected = current_breakdown)
    updateVarSelectizeInput(session, "geographic_breakdown_o2", selected = current_breakdown)
    updateVarSelectizeInput(session, "geographic_breakdown_o3", selected = current_breakdown)
    updateVarSelectizeInput(session, "geographic_breakdown_o4", selected = current_breakdown)
    updateVarSelectizeInput(session, "geographic_breakdown_e2", selected = current_breakdown)
    updateVarSelectizeInput(session, "geographic_breakdown_e3", selected = current_breakdown)
  })
