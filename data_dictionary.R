...
analyse_data_frames <- function(reload_all = FALSE) {
  ...
  if (reload_all == TRUE) {
    ...
    source(
      "C:/Users/mweller1/Documents/projects/CSC_outcomes_and_enablers/global.R"
    )
  }
  l <- ls(.GlobalEnv)
  table_defs <- Filter(
    ...
    lapply(
      ls(envir = .GlobalEnv),
      function(x) if (is.data.frame(o <- get(x))) names(o)
    )
  )
  names(table_defs) <- l[sapply(
    l,
  ...
  rbindlist(lapply(
    names(table_defs),
    function(td_name, table_defs) {
      data_types <- as.character(sapply(get(td_name), class))
      data.table(
        df_name = td_name,
        row_count = nrow(get(td_name)),
        column_order = 1:length(data_types),
        column_name = table_defs[[td_name]],
...
write.csv(
  dt_table_defs,
  "C:/Users/mweller1/OneDrive - Department for Education/Documents/CSC shiny dashboard/data_dictionary_df.csv",
  row.names = F
)
