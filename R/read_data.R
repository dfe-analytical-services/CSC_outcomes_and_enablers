...
remove_cumbria_data <- function(dataset) {
  if ("geo_breakdown" %in% names(dataset)) {
    dataset %>% filter(geo_breakdown != "Cumbria")
  }
}

redacted_to_negative <- function(
  dataset,
  col_old,
  col_new,
  copy_numeric_vals = TRUE
) {
...
get_statistical_neighbours <- function(
  file = "./data-raw/sn_model_2025_wide.csv"
) {
  ...
  setnames(
    stats_neighbours_raw,
    old = c("old_la_code", "la_name"),
    new = c("LA.number", "LA.Name")
  )
  lookup <- stats_neighbours_raw %>% select("LA.Name", "LA.number")

  setnames(
    stats_neighbours_raw,
    gsub(pattern = "SN_", "SN", names(stats_neighbours_raw))
  )

  df <- stats_neighbours_raw %>%
    select(
      "LA.Name",
      "LA.number",
      "SN1",
...
get_stats_neighbours_long <- function(stats_neighbours) {
  stats_neighbours_long <- stats_neighbours %>%
    ...
    left_join(
      stats_neighbours %>%
        select(SN_LA_name = "LA.Name", SN_LA_number = "LA.number")
    ) %>%
    as.data.table()
...
collect_summary_data_metric <- function(
  sort_order,
  dataset_name,
  ...
  dataset_out[
    value_format == "currency_per_capita",
    value := sapply(value, curreny_per_capita_format)
  ]
...
collect_summary_data_all <- function() {
  ...
  summary_data <- rbindlist(
    lapply(
      ...
      function(x, metric_parameters) {
        ...
        dimensional_filters <- eval(parse(
          text = this_metric$dimensional_filters
        ))
  ...
  date_frequency <- unique(summary_data[, .(
    tab_name,
    accordion_text,
    heading_text,
    time_period,
    metric_text
  )])[, .N, by = .(tab_name, accordion_text, heading_text, time_period)]
  date_per_heading <- merge(
    date_frequency,
    date_frequency[,
      .(N = max(N)),
      by = .(tab_name, accordion_text, heading_text)
    ]
  )
  ...
  date_per_heading <- date_per_heading[,
    .(header_time_period = max(header_time_period)),
    by = .(tab_name, accordion_text, heading_text)
  ]
  ...
  summary_data[
    header_time_period != time_period,
    metric_text := paste0(metric_text, " (", time_period, ")")
  ]
...
read_cla_rate_data <- function(
  sn_long,
  file = "./data-raw/cla_number_and_rate_per_10k_children.csv"
) {
  ...
  cla_rate_data <- cla_rate_data %>%
    ...
    filter(
      !(new_la_code %in% c("E10000009", "E10000021", "E06000028", "E06000029"))
    ) %>%
  ...
  cla_rate_data <- rbindlist(
    l = list(cla_rate_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
  ...
  cla_rate_data <- cla_rate_data %>%
    ...
    redacted_to_negative(
      col_old = "rate_per_10000",
      col_new = "Rate Per 10000"
    ) %>%
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
read_cla_placement_data <- function(
  sn_long,
  file = "./data-raw/la_children_who_started_to_be_looked_after_during_the_year.csv"
) {
...
read_cla_31_march_data <- function(
  file = "./data-raw/la_cla_on_31_march_by_characteristics.csv"
) {
...
merge_cla_dataframes <- function(sn_long) {
  ...
  cla_rates_local <- rename(cla_rates_local, rates_number = number)

  cla_placements_local <- rename(
    cla_placements_local,
  ...
  merged_data <- merged_data %>%
    mutate(
      placement_per_10000 = round(
        as.numeric(placements_number) / as.numeric(population_estimate) * 10000
      )
    ) %>%
    mutate(
      `Placement Rate Per 10000` = case_when(
  ...
  merged_data <- merged_data %>%
    mutate(
      placement_per_10000 = sapply(placement_per_10000, decimal_rounding, 0)
    ) %>%
    return(merged_data)
}
...
merge_cla_31_march_dataframes <- function(sn_long) {
  ...
  cla_31_march_data <- rename(cla_31_march_data, cla_31_march_number = number)

  # merge two data frames
  merged_31_march_data <- merge(
    cla_rates_local[geographic_level != "Statistical neighbours (median)"],
    cla_31_march_data[geographic_level != "Statistical neighbours (median)"],
    by.x = c(
      "geo_breakdown",
      "time_period",
      "geographic_level",
  ...
  merged_31_march_data <- merged_31_march_data %>%
    mutate(
      placement_per_10000 = round(
        as.numeric(cla_31_march_number) /
          as.numeric(population_estimate) *
          10000
      )
    ) %>%
    mutate(
  ...
  merged_31_march_data <- rbindlist(
    l = list(merged_31_march_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
read_cin_rate_data <- function(
  sn_long,
  file = "./data-raw/b1_children_in_need_2013_to_2025.csv"
) {
  ...
  ons_population_data <- data.table::fread(
    file = "./data-raw/ons_mid-year_population_estimates_2012_to_2024.csv"
  )
  ons_population_data <- ons_population_data[, .(
    time_period,
    country_code,
    region_code,
    old_la_code,
  ...
  cin_rate_data <- merge(cin_rate_data, ons_population_data, all.x = TRUE) %>%
    mutate(
      At31_episodes_rate = as.character(
        (as.numeric(At31_episodes) / as.numeric(population_estimate)) * 10000
      )
    ) %>%
    mutate(
      At31_episodes_rate = sapply(
        as.character(At31_episodes_rate),
        decimal_rounding,
        0
      )
    )
  ...
  cin_rate_data[
    is.na(At31_episodes_rate) & !(is.na(population_estimate)),
    At31_episodes_rate := At31_episodes
  ]
  ...
  cin_rate_data <- rbindlist(
    l = list(cin_rate_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
  ...
  cin_rate_data <- cin_rate_data %>%
    mutate(
      At31_episodes_rate = sapply(At31_episodes_rate, decimal_rounding, 0)
    ) %>%
...
# CIN referrals data
read_cin_referral_data <- function(
  sn_long,
  file = "./data-raw/c1_children_in_need_referrals_and_rereferrals_2013_to_2025.csv"
) {
  ...
  cin_referral_data <- rbindlist(
    l = list(cin_referral_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
  ...
  cin_referral_data <- cin_referral_data %>%
    mutate(
      Re_referrals_percent = sapply(Re_referrals_percent, decimal_rounding, 1)
    ) %>%
    redacted_to_negative(col_old = "Referrals", col_new = "Referrals_num") %>%
    redacted_to_negative(
      col_old = "Re_referrals",
      col_new = "Re_referrals_num"
    ) %>%
    redacted_to_na(
      col_old = "Re_referrals_percent",
      col_new = "Re_referrals_percentage"
    ) %>%
    redacted_to_negative(
      col_old = "Re_referrals_percent",
      col_new = "Re-referrals (%)"
    )
...
read_outcomes_absence_data <- function(
  sn_long,
  file = "./data-raw/absence_la.csv"
) {
  ...
  sn_metrics <- sn_aggregations(
    ...
    median_cols = c(
      ...
      "pt_pupils_pa_50_exact"
    ), # "pt_sess_authorised", "pt_sess_unauthorised"),
    sum_cols = c("t_pupils"),
    group_cols = c(
      "LA.number",
      "time_period",
  ...
  outcomes_absence_data <- rbindlist(
    l = list(outcomes_absence_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
  ...
  outcomes_absence_data[
    time_period %in% time_periods_to_update,
    (cols_to_update) := lapply(.SD, function(x) "x"),
  ...
  outcomes_absence_data <- outcomes_absence_data %>%
    select(
      ...
      pt_sess_authorised,
      t_sess_unauthorised,
      pt_sess_unauthorised,
      t_pupils_pa_10_exact,
      pt_pupils_pa_10_exact,
      t_pupils_pa_50_exact,
      pt_pupils_pa_50_exact
    ) %>%
    mutate(pt_overall = sapply(pt_overall, decimal_rounding, 1)) %>%
    mutate(
      pt_pupils_pa_10_exact = sapply(pt_pupils_pa_10_exact, decimal_rounding, 1)
    ) %>%
    mutate(
      pt_pupils_pa_50_exact = sapply(pt_pupils_pa_50_exact, decimal_rounding, 1)
    ) %>%
    mutate(
      pt_sess_authorised = sapply(pt_sess_authorised, decimal_rounding, 1)
    ) %>%
    mutate(
      pt_sess_unauthorised = sapply(pt_sess_unauthorised, decimal_rounding, 1)
    ) %>%
    redacted_to_negative(
      col_old = "pt_overall",
      col_new = "Overall absence (%)"
    ) %>%
    redacted_to_negative(
      col_old = "pt_pupils_pa_10_exact",
      col_new = "Persistent absentees (%)"
    ) %>%
    redacted_to_negative(
      col_old = "pt_pupils_pa_50_exact",
      col_new = "Severe absentees (%)"
    ) %>%
    redacted_to_negative(
      col_old = "pt_sess_authorised",
      col_new = "Authorised absence (%)"
    ) %>%
    redacted_to_negative(
      col_old = "pt_sess_unauthorised",
      col_new = "Unauthorised absence (%)"
    ) %>%
...
read_outcomes_ks2_data <- function(sn_long, file = "./data-raw/ks2_la.csv") {
  ...
  outcomes_ks2_data <- rbindlist(
    l = list(outcomes_ks2_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
  ...
  outcomes_ks2_data <- outcomes_ks2_data %>%
    ...
    select(
      ...
      social_care_group,
      version,
      t_read_eligible_pupils,
      t_read_met_expected_standard,
      pt_read_met_expected_standard,
      t_writta_eligible_pupils,
      t_writta_met_expected_standard,
      ...
      t_rwm_met_expected_standard,
      pt_rwm_met_expected_standard,
      t_gps_eligible_pupils,
      t_gps_met_expected_standard,
      pt_gps_met_expected_standard,
      t_scita_eligible_pupils,
      t_scita_met_expected_standard,
      pt_scita_met_expected_standard,
      t_read_progress_eligible_pupils,
      t_read_progress_score,
      avg_read_progress_score,
      avg_read_progress_score_lower_CI,
      avg_read_progress_score_upper_CI,
      t_writta_progress_eligible_pupils,
      t_writta_progress_score,
      avg_writta_progress_score,
      avg_writta_progress_score_lower_CI,
      avg_writta_progress_score_upper_CI,
      t_mat_progress_eligible_pupils,
      t_mat_progress_score,
      avg_mat_progress_score,
      avg_mat_progress_score_lower_CI,
      avg_mat_progress_score_upper_CI
    ) %>%
    mutate(
      pt_rwm_met_expected_standard = sapply(
        pt_rwm_met_expected_standard,
        decimal_rounding,
        0
      )
    ...
    # Make % columns numeric
    redacted_to_negative(
      col_old = "pt_rwm_met_expected_standard",
      col_new = "Expected standard reading writing maths (%)"
    )
...
read_outcomes_ks4_data <- function(sn_long, file = "./data-raw/ks4_la.csv") {
  ...
  outcomes_ks4_data <- rbindlist(
    l = list(outcomes_ks4_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
read_school_stability_data <- function(
  sn_long,
  file = "./data-raw/la_cla_school_moves.csv"
) {
  ...
  sch_stability_data <- rbindlist(
    l = list(sch_stability_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  ) %>%
...
read_outcome2 <- function(
  sn_long,
  file = "./data-raw/la_children_who_ceased_during_the_year.csv"
) {
  ...
  ceased_cla_data <- rbindlist(
    l = list(ceased_cla_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
read_cpp_in_year_data <- function(
  sn_long,
  file = "./data-raw/d3_cpps_subsequent_plan_2013_to_2025.csv"
) {
  ...
  cpp_in_year_data <- rbindlist(
    l = list(cpp_in_year_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
  ...
  cpp_in_year_data <- cpp_in_year_data %>%
    ...
    redacted_to_negative(
      col_old = "CPP_subsequent_percent",
      col_new = "Repeat_CPP_percent"
    )
...
read_cpp_by_duration_data <- function(
  sn_long,
  file = "./data-raw/d5_cpps_at31march_by_duration_2013_to_2025.csv"
) {
  ...
  cpp_by_duration_data <- rbindlist(
    l = list(cpp_by_duration_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
  ...
  cpp_by_duration_data <- cpp_by_duration_data %>%
    mutate(
      X2_years_or_more_percent = sapply(
    ...
    select(
      ...
      CPP_At31,
      `X3_months_or_less`,
      `X3_months_or_less_percent`,
      more_than_3_months_6_months,
      more_than_3_months_6_months_percent,
      more_than_6_months_less_than_1_year,
      more_than_6_months_less_than_1_year_percent,
      `X1_year_less_than_2_years`,
      `X1_year_less_than_2_years_percent`,
      `X2_years_or_more`,
      `X2_years_or_more_percent`
    ) %>%
    redacted_to_negative(
      col_old = "X2_years_or_more_percent",
      col_new = "CPP_2_years_or_more_percent"
    )
...
read_a_and_e_data <- function(
  ...
  la_admissions <- la_admissions %>%
    ...
    filter(
      !(Area.Name %in%
        c("Dorset", "Bournemouth, Christchurch and Poole") &
        Time.period < "2019")
    ) %>%
  ...
  admissions_data_joined <- rbind(
    ...
    rename(
      ...
      `geographic_level` = `Area.Type`,
      `geo_breakdown` = `AreaName`,
      `new_la_code` = `Area.Code`
    ) %>%
    distinct()

  admissions_data_joined["geographic_level"][
    admissions_data_joined["geographic_level"] == "Regions (statistical)"
  ] <- "Regional"
  admissions_data_joined["geographic_level"][
    admissions_data_joined["geographic_level"] ==
  ...
  admissions_data_joined["geographic_level"][
    admissions_data_joined["geographic_level"] == "England"
  ] <- "National"
  admissions_data_joined["geo_breakdown"][
    admissions_data_joined["geo_breakdown"] == "England"
  ] <- "National"
  ...
  admissions_data2 <- left_join(
    admissions_data_joined,
    la_codes,
    by = c("new_la_code")
  )
  ...
  admissions_data2 <- admissions_data2 %>%
    mutate(
      geo_breakdown = case_when(
        geo_breakdown == "Yorkshire and the Humber" ~
  ...
  admissions_data3 <- rbindlist(
    l = list(admissions_data2, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
### Assessment Factors ------
read_assessment_factors <- function(
  sn_long,
  file = "./data-raw/c3_factors_identified_at_end_of_assessment_2018_to_2025.csv"
) {
  ...
  columns <- c(
    ...
    "Drug_Misuse_parent",
    "Drug_Misuse_person",
    "Domestic_Abuse_child",
    "Domestic_Abuse_parent",
    "Domestic_Abuse_person",
    "Mental_Health_child",
    ...
    "Neglect",
    "Emotional_Abuse",
    "Physical_Abuse_unknown",
    "Physical_Abuse_child_on_child",
    "Physical_Abuse_adult_on_child",
    "Sexual_Abuse_unknown",
    "Sexual_Abuse_child_on_child",
    "Sexual_Abuse_adult_on_child",
  ...
  ass_fac_data <- left_join(
    ...
    mutate(
      `rate_per_10000` = (as.numeric(value) / as.numeric(population_estimate)) *
        10000
    ) %>%
    mutate(
      `rate_per_10000` = sapply(
        as.character(rate_per_10000),
        decimal_rounding,
        0
      )
    ) %>%
  ...
  ass_fac_data <- rbindlist(
    l = list(ass_fac_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
  ...
  ass_fac_data <- ass_fac_data %>%
    ...
    redacted_to_negative(
      col_old = "rate_per_10000_char",
      col_new = "rate_per_10000"
    ) %>%
...
read_number_placements_data <- function(
  sn_long,
  file = "./data-raw/la_cla_placement_stability.csv"
) {
  ...
  placement_chg_data <- rbindlist(
    l = list(placement_chg_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
read_placement_info_data <- function(
  sn_long,
  file = "./data-raw/la_cla_on_31_march_by_characteristics.csv"
) {
  ...
  placement_info_data <- placement_info_data %>%
    ...
    filter(
      cla_group %in% c("Placement", "Distance between home and placement")
    ) %>%
    insert_geo_breakdown() %>%
    remove_cumbria_data()
  ...
  placement_info_data <- rbindlist(
    l = list(placement_info_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
read_care_leavers_activity_data <- function(
  sn_long,
  file = "./data-raw/la_care_leavers_activity.csv"
) {
  ...
  cl_activity_data <- rbindlist(
    l = list(cl_activity_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
read_care_leavers_accommodation_suitability <- function(
  sn_long,
  file = "./data-raw/la_care_leavers_accommodation_suitability.csv"
) {
  ...
  cl_accom_data <- rbindlist(
    l = list(cl_accom_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
read_wellbeing_child_data <- function(
  sn_long,
  file = "./data-raw/la_conviction_health_outcome_cla.csv"
) {
  data <- fread(file)
  ...
  data4 <- data3 %>%
    ...
    mutate(
      sdq_score_recd_x_score = case_when(
        characteristic == "SDQ average score" ~
          (as.numeric(sdq_score_recd) * as.numeric(number)),
        TRUE ~ NA
      )
    )
...
read_placement_order_match_data <- function(
  file = "./data-raw/national_cla_adopted_average_time_between_adoption_process_stages.csv"
) {
  data <- read.csv(file)
...
read_social_worker_stability_data <- function(
  sn_long,
  file = "./data-raw/la_cla_swcount.csv"
) {
  # load the file into a data.table
  ...
  sn_metrics <- sn_aggregations(
    sn_long = sn_long,
    dataset = sw_stability_data,
    median_cols = c("percentage"),
    sum_cols = c("number"),
    group_cols = c("LA.number", "time_period", "cla_group", "sw_stability")
  )

  sw_stability_data <- rbindlist(
    l = list(sw_stability_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  ) %>%
    mutate(percentage = sapply(percentage, decimal_rounding, 0)) %>%
...
read_workforce_data <- function(
  sn_long,
  file = "./data-raw/csww_indicators_2017_to_2025.csv"
) {
  ...
  workforce_data <- workforce_data %>%
    ...
    select(
      ...
      "absence_rate_fte",
      "agency_rate_fte",
      "agency_cover_rate_fte",
      "vacancy_rate_fte",
      "vacancy_agency_cover_rate_fte",
      "turnover_rate_headcount",
      "agency_rate_headcount",
      "caseload_fte"
    )
  ...
  workforce_data <- rbindlist(
    l = list(workforce_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
  ...
  workforce_data[
    geographic_level == "Local authority",
    old_la_code := (original_old_la_code)
  ]
  ...
  workforce_data <- workforce_data %>%
    ...
    filter(!(new_la_code %in% dropList)) %>%
    mutate(
      turnover_rate_fte = sapply(turnover_rate_fte, decimal_rounding, 1)
    ) %>%
  ...
  workforce_data <- suppressWarnings(
    workforce_data %>%
      mutate(across(
...
read_workforce_eth_data <- function(
  sn_long,
  file = "./data-raw/csww_role_by_characteristics_inpost_2019_to_2025.csv"
) {
  ...
  workforce_ethnicity_data$new_la_code[
    workforce_ethnicity_data$new_la_code == ""
  ] <- NA
  workforce_ethnicity_data$region_code[
    workforce_ethnicity_data$region_code == ""
  ] <- NA
  workforce_ethnicity_data <- mutate(
    workforce_ethnicity_data,
    code = coalesce(new_la_code, region_code, country_code)
  )

  workforce_ethnicity_data <- convert_perc_cols_to_numeric(as.data.frame(
    workforce_ethnicity_data
  ))

  # now we need to create a Non-white set of metrics
  setDT(workforce_ethnicity_data)
  non_white_data <- workforce_ethnicity_data[
    breakdown == "White" & role == "Total"
  ]
  non_white_data[
    is.numeric(as.numeric(inpost_headcount_percentage)),
    inpost_headcount_percentage := as.character(
      100 - as.numeric(inpost_headcount_percentage)
    )
  ]
  non_white_data[, breakdown := "Non-white"]

  # and then append these to the dataset
  workforce_ethnicity_data <- rbindlist(list(
    workforce_ethnicity_data,
    non_white_data
  ))
  ...
  final_dataset <- rbindlist(
    l = list(workforce_ethnicity_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
  ...
  final_dataset <- final_dataset %>%
    mutate(
      `inpost_headcount_percentage` = sapply(
        `inpost_headcount_percentage`,
        decimal_rounding,
        1
      )
...
read_workforce_eth_seniority_data <- function(
  file = "./data-raw/csww_role_by_characteristics_inpost_2019_to_2025.csv"
) {
  ...
  workforce_ethnicity_seniority_data <- subset(
    workforce_ethnicity_seniority_data,
  ...
  workforce_ethnicity_seniority_data$new_la_code[
    workforce_ethnicity_seniority_data$new_la_code == ""
  ] <- NA
  workforce_ethnicity_seniority_data$region_code[
    workforce_ethnicity_seniority_data$region_code == ""
  ] <- NA
  workforce_ethnicity_seniority_data <- mutate(
    workforce_ethnicity_seniority_data,
    code = coalesce(new_la_code, region_code, country_code)
  )
  ...
  workforce_ethnicity_seniority_data <- workforce_ethnicity_seniority_data %>%
    mutate(
      seniority = case_when(
        ...
        role %in% c("First line manager", "Senior manager", "Middle manager") ~
          "Manager"
      )
    )
  ...
  workforce_ethnicity_seniority_data <- left_join(
    workforce_ethnicity_seniority_data,
    total_observation,
    by = c(
      "geographic_level",
...
read_ethnic_population_data <- function(
  file1 = "./data-raw/ons-ethnic-population-reg.csv",
  file2 = "./data-raw/ons-ethnic-population-nat.csv",
  file3 = "./data-raw/ons-ethnic-population-la.csv"
) {
  # Read the csv files
  ...
  names(df_Kingston_upon_Thames) <- c(
    "Code",
    "Name",
    "EthnicGroupCode",
    "EthnicGroup",
  ...
  names(df_North_Northamptonshire) <- c(
    "Code",
    "Name",
    "EthnicGroupCode",
    "EthnicGroup",
  ...
  df_Kingston_upon_Thames <- df_Kingston_upon_Thames %>%
    mutate(
      Name = "Kingston upon Thames / Richmond upon Thames",
  ...
  df_North_Northamptonshire <- df_North_Northamptonshire %>%
    mutate(
      Name = "North Northamptonshire / West Northamptonshire",
      Code = "E06000061 / E06000062"
    ) %>%
    group_by(Code, Name, EthnicGroupCode, EthnicGroup, geographic_level) %>%
    summarise(Observation = sum(Observation), .groups = "drop")
  ...
  ethnic_population_data <- rbind(
    df_regions,
    df_countries,
    df_authorities,
    df_Inner_London,
    df_Outer_London,
    df_Kingston_upon_Thames,
    df_North_Northamptonshire
  )
  ethnic_population_data <- subset(
    ethnic_population_data,
    select = -c(EthnicGroupCode)
  )

  # Remove rows where 'EthnicGroup' equals 'Does not apply'
  ethnic_population_data <- ethnic_population_data[
    ethnic_population_data$EthnicGroup != "Does not apply",
  ]

  # Convert 'EthnicGroup' to character type
  ethnic_population_data$EthnicGroup <- as.character(
    ethnic_population_data$EthnicGroup
  )
  ...
  # Join the total observation back to the original dataframe
  ethnic_population_data <- left_join(
    ethnic_population_data,
    total_observation,
    by = c("Name", "geographic_level")
  )
...
merge_eth_dataframes <- function(sn_long) {
  ...
  if (!exists("workforce_eth")) {
    ...
    workforce_eth_local <- read_workforce_eth_data(sn_long = sn_long)[
  ...
  merged_data <- left_join(
    workforce_eth_local,
    population_eth,
    by = c("code" = "Code", "breakdown" = "EthnicGroupShort")
  )
...
read_spending_data <- function(
  ...
  data2 <- data %>%
    select(
      ...
      "Detailed Class",
      "Certification",
      "Children Social Care - Total Expenditure (C3 = C1 + C2)",
      "Total Service Expenditure - Total Expenditure (C3 = C1 + C2)"
    )

  data3 <- data2 %>%
    filter(data2$Class %in% c("UA", "MD", "LB", "SC", "Eng")) %>%
    rename(
      `CS Expenditure` = "Children Social Care - Total Expenditure (C3 = C1 + C2)",
      `Total Expenditure` = "Total Service Expenditure - Total Expenditure (C3 = C1 + C2)"
    ) %>%
  ...
  london_com <- regional_spending %>%
    filter(
      geo_breakdown == "Inner London" | geo_breakdown == "Outer London"
    ) %>%
    summarise(
      exp = sum(exp),
      total_exp = sum(total_exp),
  ...
  final_dataset <- rbindlist(
    l = list(final_dataset, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
read_population_estimates <- function(file = "./data-raw/mye24tablesew.xlsx") {
  ...
  test_df$under18 <- rowSums(test_df[, c(
    "0",
    "1",
    "2",
    "3",
  ...
  local_authority_pop <- test_df2 %>%
    select("Code", "Name", "Geography", "under18")

  regional_pop <- test_df[grepl("^E12", test_df$Code), ]
  regional_pop$Name <- str_to_title((regional_pop$Name))
  regional_pop$Name[
    regional_pop$Name == "Yorkshire And The Humber"
  ] <- "Yorkshire and The Humber"
  ...
  # Get national data
  national_pop <- sum(
    population[population$Geography == "Region", "under18"],
    na.rm = FALSE
  )
  new_row <- data.frame(
    Code = "E92000001",
  ...
  inner_london <- sum(
    population3[(population3$new_la_code %in% inner_london_list), "under18"],
    na.rm = FALSE
  )
  outer_london <- sum(
    population3[(population3$new_la_code %in% outer_london_list), "under18"],
    na.rm = FALSE
  )
  inner_row <- data.frame(
    new_la_code = "",
    geographic_level = "Regional",
    geo_breakdown = "Inner London",
    under18 = inner_london
  )
  outer_row <- data.frame(
    new_la_code = "",
    geographic_level = "Regional",
    geo_breakdown = "Outer London",
    under18 = outer_london
  )
...
read_per_capita_spending <- function(sn_long) {
  ...
  joined_data <- left_join(
    spending_data[geographic_level != "Statistical neighbours (median)"],
    population_estimates,
    by = c("geographic_level", "geo_breakdown")
  )
  joined_data$`Cost per child` <- format(
    (joined_data$exp / joined_data$under18) * 1000,
    digits = 1
  )
  joined_data$cost_per_capita <- format(
    (joined_data$exp / joined_data$under18) * 1000,
    digits = 1
  )
  ...
  joined_data2 <- joined_data %>%
    mutate(
      `Cost per child` = case_when(
        exp == -300 ~ "x",
        TRUE ~ as.character(`Cost per child`)
      )
    ) %>%
    mutate(
      cost_per_capita = case_when(
        exp == -300 ~ -300,
        TRUE ~ as.numeric(cost_per_capita)
      )
    )

  joined_data2$`Cost per child` <- formatC(
    joined_data2$`Cost per child`,
    format = "f",
    big.mark = ",",
    digits = 0
  )
  joined_data2$cost_per_capita <- janitor::round_half_up(
    joined_data2$cost_per_capita
  )
  ...
  final_dataset <- rbindlist(
    l = list(joined_data2, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
read_spending_data2 <- function(
  ...
  data2 <- data %>%
    select(
      ...
      "Detailed Class",
      "Certification",
      "TOTAL CHILDREN SOCIAL CARE - Total Expenditure (C3 = C1 + C2)",
      "Children's social care - Children Looked After - Total Expenditure (C3 = C1 + C2)"
    )

  data3 <- data2 %>%
    filter(data2$Class %in% c("UA", "MD", "LB", "SC", "Eng")) %>%
    rename(
      `CLA Expenditure` = "Children's social care - Children Looked After - Total Expenditure (C3 = C1 + C2)",
      `Total Expenditure` = "TOTAL CHILDREN SOCIAL CARE - Total Expenditure (C3 = C1 + C2)"
    ) %>%
    # replace "[x]" values with x
    mutate_all(~ gsub("\\[x\\]", "x", .)) %>%
    # replace & in local authority names with "and"
  ...
  data3$minus_cla_share <- ((data3$total_exp - data3$cla_exp) /
    (data3$total_exp)) *
    100

  data3 <- data3 %>%
    mutate(
      minus_cla_share = case_when(
        `CLA Expenditure` == "x" ~ 0,
  ...
  df2[
    `CLA Expenditure` == "x" | `Total Expenditure` == "x",
    minus_cla_share := NA
  ]
  ...
  final_dataset <- final_dataset %>%
    mutate(
      minus_cla_share = sapply(
        as.character(minus_cla_share),
        decimal_rounding,
        1
      )
    ) %>%
    mutate(
      `Excluding CLA Share` = case_when(
...
read_ofsted_leadership_data <- function(
  ...
  ofsted_leadership_data$`Inspection date` <- as.Date(
    ofsted_leadership_data$`Inspection date`,
    format = "%d/%m/%Y"
  )
  ofsted_leadership_data$inspection_year <- format(
    ofsted_leadership_data$`Inspection date`,
    "%Y"
  )
  ofsted_leadership_data$published_year <- year(as.Date(
    ofsted_leadership_data$`Publication date`,
    format = "%d/%m/%Y"
  ))
  ofsted_leadership_data$time_period <- max(format(
    ofsted_leadership_data$`Inspection date`,
    "%Y"
  ))
  ...
  ofsted_leadership_data <- ofsted_leadership_data %>%
    ...
    mutate(
      geo_breakdown = case_when(
        ...
        geo_breakdown == "Bournemouth, Christchurch & Poole" ~
          "Bournemouth, Christchurch and Poole",
        geo_breakdown == "Herefordshire" ~ "Herefordshire, County of",
        geo_breakdown == "Hammersmith & Fulham" ~ "Hammersmith and Fulham",
        geo_breakdown %in% c("Kingston Upon Hull", "Kingston upon Hull") ~
  ...
  ofsted_region_corrections <- fread(
    "./data-raw/ofsted_region_mapping_corrections.csv"
  )

  ofsted_leadership_data <- merge(
    ofsted_leadership_data,
    ofsted_region_corrections,
    by.x = "geo_breakdown",
    by.y = "la_name",
    all.x = TRUE
  )
  ofsted_leadership_data[
    !is.na(region_name_correct),
    region := region_name_correct
  ][, region_name_correct := NULL]
  ...
  cla_rates_selected <- read_cla_rate_data(sn_long = sn_long) %>%
    select(geo_breakdown, old_la_code)
  cla_rates_selected <- cla_rates_selected %>%
    distinct(geo_breakdown, old_la_code, .keep_all = TRUE)
  ofsted_leadership_data <- left_join(
    ofsted_leadership_data,
    cla_rates_selected,
    by = c("geo_breakdown" = "geo_breakdown")
  )

  ofsted_leadership_data <- ofsted_leadership_data %>%
    mutate(
      inadequate_count = ifelse(impact_of_leaders == "Inadequate", 1, 0),
      requires_improvement_count = ifelse(
        impact_of_leaders == "Requires improvement to be good",
        1,
        0
      ),
  ...
  ofsted_leadership_data <- bind_rows(
    ofsted_leadership_data,
    region_counts,
    london_counts,
    national_counts
  ...
  ofsted_leadership_data <- rbindlist(
    l = list(ofsted_leadership_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  )
...
read_s47_to_ICPC_data <- function(
  sn_long,
  file = "./data-raw/c4_children_in_need_section_47s_and_icpcs_2013_to_2025.csv"
) {
  # load the file into a data.table
  ...
  s47_to_ICPC_data <- s47_to_ICPC_data %>%
    mutate(
      percentage = as.character(
        (as.numeric(ICPC) / as.numeric(Section47) * 100)
      )
    ) %>%
  ...
  s47_to_ICPC_data <- rbindlist(
    l = list(s47_to_ICPC_data, sn_metrics),
    fill = TRUE,
    use.names = TRUE
  ) %>%
    mutate(percentage = sapply(percentage, decimal_rounding, 1)) %>%
