...
if (TRUE == FALSE) {
  # this IF statement is to prevent the following block of code from running if sourced
  ...
  pipeline_prelim <- get_pipeline_prelim(
    path_new = paste0(YOUR_LOCAL_PATH, TASK_NAME, "/data-comparisons/2025/"),
    path_old = paste0(YOUR_LOCAL_PATH, TASK_NAME, "/data-comparisons/2024/")
  )
  ...
  rmarkdown::render(
    ...
    params = list(
      ...
      pipeline_comparison_file = paste0(
        YOUR_LOCAL_PATH,
        "pipeline_comparison_prelim.rds"
      )
    )
  )
  ...
  saveRDS(
    pipeline_run$pipeline_comparison,
    file = paste0(YOUR_LOCAL_PATH, "pipeline_comparison.rds")
  )
  ...
  pipeline_run$pipeline_comparison$consolidated_field_diffs$ks4_la[
    !(variable_clean %in% c("avg_att8", "t_pupils"))
  ]
  ...
  reason_for_pipeline_run <- "Update ONS population estimate data" # <---- EDIT HERE
