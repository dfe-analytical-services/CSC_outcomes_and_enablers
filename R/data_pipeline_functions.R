...
get_pipeline_prelim <- function(path_new, path_old) {
  ...
  file_name_mismatches <- file_name_matches[which(
    !(file_name_matches %in% files_new)
  )]

  # get the old files read in from csv to a data.table
  old <- lapply(
    file_name_matches,
    function(file_name) fread(paste0(path_old, file_name))
  )
  ...
  new <- lapply(
    file_name_matches,
    function(file_name) fread(paste0(path_new, file_name))
  )
...
run_data_pipeline_step_1 <- function(
  ...
  pipeline_comparison <- pipeline_compare_datasets(
    meta_rds,
    meta_new,
    datasets_rds,
    datasets_new
  ...
  if (save_comparison == TRUE) {
    ...
    writexl::write_xlsx(
      x = pipeline_comparison$consolidated_setdiffs_summary,
      path = paste0(
        YOUR_LOCAL_PATH,
        TASK_NAME,
        "/consolidated_set_diffs_summary_",
        TASK_NAME,
        "_",
        PIPELINE_RUN_VERSION,
        ".xlsx"
    ...
    # detailed setdiffs
    deltas_to_export <- rlang::flatten(
      pipeline_comparison$consolidated_setdiffs
    )
    ...
    deltas_to_export <- rlang::flatten(
      pipeline_comparison$consolidated_field_diffs
    )
    names(deltas_to_export)
    writexl::write_xlsx(
      deltas_to_export,
...
run_data_pipeline_step_2 <- function(pipeline_run, pipeline_run_parameters) {
  ...
  if (!nchar(pipeline_run_parameters$username) > 0) {
    return(
      "pipeline_run_parameters must have a username with more than zero chars"
    )
  }
  # expect a run_date_time in the right format
  if (
    !is(lubridate::as_datetime(pipeline_run_parameters$run_datetime), "POSIXct")
  ) {
    return(
      "pipeline_run_parameters must have a run_datetime in the right format"
    )
  }
  ...
  if (
    !identical(names(pipeline_run), c("datasets_new", "pipeline_comparison"))
  ) {
  ...
  user_input <- readline(
    prompt = "Data pipeline step 2 checks passed, are you sure you wish to finalise the update? Type 'Yes' to proceed: "
  )
  if (user_input != "Yes") {
    return(paste0(
      "Pipeline step 2 aborted.  Dashboard datasets not overwritten.  You entered '",
      user_input,
      "', to proceed you are required to enter (case-sensitive) 'Yes'."
    ))
  }
  ...
  for (dataset_name in names(pipeline_run$datasets_new)) {
    ...
    print(paste(
      dataset_name,
      " ---copied to---> ",
      paste0("./data/", dataset_name, ".rds")
    ))
    # write the dataset out to rds file
    saveRDS(
      object = pipeline_run$datasets_new[[dataset_name]],
      file = paste0("./data/", print(dataset_name), ".rds")
    )
  }
...
pipeline_generate_datasets <- function() {
  ...
  sw_stability_data <- read_social_worker_stability_data(
    sn_long = stats_neighbours_long
  ) # new metric
  ...
  school_stability_data <- suppressWarnings(read_school_stability_data(
    sn_long = stats_neighbours_long
  ))
  ...
  s47_to_ICPC_data <- suppressWarnings(read_s47_to_ICPC_data(
    sn_long = stats_neighbours_long
  )) # new metric
...
pipeline_compare_datasets <- function(
  ...
  dataset_name_comparison <- dcast.data.table(
    rbindlist(list(
      data.table(
        rds_or_new = "CURRENT",
        check_type = "dataset_name",
        dataset_name = meta_rds$dataset_names
      ),
      data.table(
        rds_or_new = "NEW",
        check_type = "dataset_name",
        dataset_name = meta_new$dataset_names
      )
    )),
    dataset_name ~ check_type + rds_or_new
  )[, match_dataset_name := dataset_name_NEW == dataset_name_CURRENT]

  dataset_class_comparison <- dcast.data.table(
    rbindlist(list(
      data.table(
        rds_or_new = "CURRENT",
        check_type = "dataset_class",
        dataset_name = names(meta_rds$dataset_class),
        dataset_class = sapply(meta_rds$dataset_class, paste, collapse = ", ")
      ),
      data.table(
        rds_or_new = "NEW",
        check_type = "dataset_class",
        dataset_name = names(meta_new$dataset_class),
        dataset_class = sapply(meta_new$dataset_class, paste, collapse = ", ")
      )
    )),
    dataset_name ~ check_type + rds_or_new
  )[, match_dataset_class := dataset_class_NEW == dataset_class_CURRENT]

  dataset_nrow_comparison <- dcast.data.table(
    rbindlist(list(
      data.table(
        rds_or_new = "CURRENT",
        check_type = "num_rows",
        dataset_name = names(meta_rds$dataset_nrow),
        dataset_nrow = sapply(meta_rds$dataset_nrow, paste, collapse = ", ")
      ),
      data.table(
        rds_or_new = "NEW",
        check_type = "num_rows",
        dataset_name = names(meta_new$dataset_class),
        dataset_nrow = sapply(meta_new$dataset_nrow, paste, collapse = ", ")
      )
    )),
    dataset_name ~ check_type + rds_or_new
  )[, match_dataset_num_rows := num_rows_NEW == num_rows_CURRENT]

  dataset_columns_comparison <- dcast.data.table(
    rbindlist(list(
      data.table(
        rds_or_new = "CURRENT",
        check_type = "dataset_columns",
        dataset_name = names(meta_rds$dataset_columns),
        dataset_columns = sapply(
          meta_rds$dataset_columns,
          paste,
          collapse = ", "
        )
      ),
      data.table(
        rds_or_new = "NEW",
        check_type = "dataset_columns",
        dataset_name = names(meta_new$dataset_columns),
        dataset_columns = sapply(
          meta_new$dataset_columns,
          paste,
          collapse = ", "
        )
      )
    ...
    dataset_name ~ check_type + rds_or_new
  )[, match_dataset_columns := dataset_columns_NEW == dataset_columns_CURRENT]

  datasets_to_compare <- dataset_name_comparison[
    match_dataset_name == TRUE
  ]$dataset_name
  dataset_geographies_comparison <- compare_dataset_geographies(
    datasets_to_compare,
    datasets_new,
    datasets_rds
  )
  ...
  dataset_comparison_summary[,
    match_summary := ((match_dataset_name +
      ...
      match_dataset_columns) ==
      4)
  ]

  # objects consistent in both (i.e. setdiff will not fail)
  diff_datasets <- dataset_comparison_summary[
    match_dataset_name + match_dataset_class + match_dataset_columns == 3
  ]$dataset_name

  # add in a data comparison (setdiffs)
  df_setdiffs <- lapply(
    diff_datasets,
    function(df_name, datasets_new, datasets_rds) {
      print(df_name)
      list(
  ...
  changed_datasets <- names(which(
    lapply(df_setdiffs, function(x) nrow(x$old_v_new) + nrow(x$new_v_old)) > 0
  ))
  ...
  if (length(changed_datasets) > 0) {
    ...
    candidate_key_cols <- c(
      "time_period",
      "old_la_code",
      dataset_column_values_comparison$new[number_count == 0]$column_name
    )
    consolidated_setdiffs <- lapply(
      changed_datasets,
      function(dataset_name, df_setdiffs, candidate_key_cols) {
        added <- setDT(df_setdiffs[[dataset_name]]$new_v_old)[, new := "NEW"]
        removed <- setDT(df_setdiffs[[dataset_name]]$old_v_new)[, old := "OLD"]
        join_cols <- intersect(
          intersect(names(added), names(removed)),
          candidate_key_cols
        )
        dataset_compare <- merge(
          ...
          by = join_cols,
          all = TRUE,
          suffixes = c("_newval", "_oldval")
        )
        new_order <- c(
          join_cols,
          sort(setdiff(names(dataset_compare), join_cols))
        )
    ...
    csd_value_counts <- rbindlist(lapply(
      names(consolidated_setdiffs),
      function(this_dataset_name, consolidated_setdiffs) {
        csd <- consolidated_setdiffs[[this_dataset_name]]
        rbindlist(lapply(
          names(csd),
          function(column_name, csd, this_dataset_name) {
    ...
    summary_csd <- dcast.data.table(
      csd_value_counts,
      dataset_name + column_name + value ~ new_or_old,
      value.var = "count"
    )
  ...
  rbindlist(
    lapply(
      ...
      function(x, datasets_new, datasets_rds) {
        if (
          "geo_breakdown" %in%
            names(datasets_new[[x]]) &
            "geo_breakdown" %in% names(datasets_rds[[x]])
        ) {
          geo_column <- "geo_breakdown"
        } else if (
          "la_name" %in%
            names(datasets_new[[x]]) &
            "la_name" %in% names(datasets_rds[[x]])
        ) {
          geo_column <- "la_name"
        } else {
          geo_column <- NULL
        ...
        if (!is.null(geo_column)) {
          las_added <- paste0(
            setdiff(
              unique(datasets_new[[x]][[geo_column]]),
              unique(datasets_rds[[x]][[geo_column]])
            ),
            collapse = ", "
          )
          las_removed <- paste0(
...
summarise_columns_over_datasets <- function(
  ...
  column_values <- rbindlist(lapply(
    names(all_data),
    function(column_name, all_data) {
      data.table(
        column_name = column_name,
        column_value = unique(all_data[[column_name]])
      )
    },
    all_data
  ))
  column_values$numeric_value <- sapply(
    column_values$column_value,
    function(x) !is.na(as.numeric(x))
  )

  column_summary <- merge(
    column_values[,
      .(unique_value_count = .N, number_count = sum(numeric_value)),
      by = .(column_name)
    ],
    column_names[,
      .(
        occurences = .N,
        row_count = sum(num_rows),
        sum_unique_value_count = sum(unique_value_count)
      ),
      by = column_name
    ]
...
field_diffs <- function(csd) {
  ...
  csd_melt[
    grep("_oldval", variable),
    variable_clean := gsub("_oldval", "", variable)
  ]
  csd_melt[
    grep("_newval", variable),
    variable_clean := gsub("_newval", "", variable)
  ]
  ...
  this_formula <- as.formula(paste0(
    paste(c(id_vars, "variable_clean"), collapse = " + "),
    "~ ",
    "old_or_new"
  ))
    ...
    lapply(
      ...
      function(x, history_list) {
        ...
        list(
          ...
          "reason_for_pipeline_run" = history_list[[
            x
          ]]$parameters$reason_for_pipeline_run,
...
consolidate_pipeline_history <- function() {
  ...
  other_history_path <- paste0(
    YOUR_LOCAL_PATH,
    TASK_NAME,
    "/data_pipeline_run_history.rds"
  )
