...
output_data <- create_qa_dataset_sn(
  output_identifier,
...
dataset_raw <- read_cla_rate_data(sn_long = stats_neighbours_long)[
...
dataset_raw <- read_outcome2(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical neighbours (median)"
]
...
output_data <- create_qa_dataset_sn(
  ...
  raw_qa_formula_2,
  write_data = TRUE,
  measure.vars_calc = measure.vars_calc,
  dataset_raw = cin_referral_data
)
...
dataset_raw <- read_number_placements_data(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical neighbours (median)"
]
id.vars_raw <- c(
  "time_period",
...
dataset_raw <- read_placement_info_data(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical neighbours (median)"
]
id.vars_raw <- c(
  "time_period",
...
dataset_raw <- read_care_leavers_activity_data(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical neighbours (median)"
]
...
dataset_raw <- read_care_leavers_accommodation_suitability(
...
dataset_raw <- read_spending_data(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical_neighbours (median)"
]
...
# csv_file <- "./data/csww_indicators_2017_to_2024.csv"
dataset_raw <- read_per_capita_spending(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical neighbours (median)"
]
...
# csv_file <- "./data/csww_indicators_2017_to_2024.csv"
dataset_raw <- read_spending_data2(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical neighbours (median)"
]
...
# csv_file <- "./data/csww_indicators_2017_to_2024.csv"
dataset_raw <- read_ofsted_leadership_data(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical neighbours (median)"
]
...
dataset_calc <- copy(workforce_eth)[
  role == "Total" & breakdown == "Non-white" & time_period == "2025"
]
# csv_file <- "./data/csww_indicators_2017_to_2025.csv"
dataset_raw <- read_workforce_eth_data(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical neighbours (median)"
][role == "Total" & breakdown == "Non-white" & time_period == "2025"]
...
# csv_file <- "./data/csww_indicators_2017_to_2024.csv"
dataset_raw <- read_wellbeing_child_data(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical neighbours (median)"
][characteristic == "SDQ average score"] # & time_period == "2024"]
...
# csv_file <- "./data/csww_indicators_2017_to_2024.csv"
dataset_raw <- read_a_and_e_data(sn_long = stats_neighbours_long)[
  geographic_level != "Statistical neighbours (median)"
]
...
admissions_data_joined["geographic_level"][
  admissions_data_joined["geographic_level"] == "Counties & UAs (from Apr 2023)"
] <- "Local authority"
admissions_data_joined["geographic_level"][
...
dataset_calc <- copy(combined_cla_data)[time_period <= "2025"][
  characteristic == "UASC"
][population_count == "Children starting to be looked after each year"]

dataset_raw <- suppressWarnings(merge_cla_dataframes(
  sn_long = stats_neighbours_long
))[geographic_level != "Statistical neighbours (median)"][
  time_period <= "2025"
][characteristic == "UASC"][
...
dataset_calc <- copy(combined_cla_31_march_data)[time_period <= "2025"][
  characteristic == "UASC"
][population_count == "Children looked after on 31 March each year"]

dataset_raw <- suppressWarnings(merge_cla_31_march_dataframes(
  sn_long = stats_neighbours_long
))[geographic_level != "Statistical neighbours (median)"][
  time_period <= "2025"
][characteristic == "UASC"][
...
dataset_calc <- copy(sw_stability_data)[time_period <= "2024"][
  cla_group == "CLA on 31 March"
][sw_stability == "3 or more social workers during the year"]

dataset_raw <- sw_stability_data[geographic_level == "Local authority"][
  time_period <= "2024"
][cla_group == "CLA on 31 March"][
  sw_stability == "3 or more social workers during the year"
]
