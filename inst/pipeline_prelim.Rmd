...
params:
  ...
  pipeline_comparison_file: '~/CSC shiny dashboard/Data QA/pipeline_comparison_prelim.rds'
---
...
if (is.null(params$pipeline_comparison)) {
  pc <- readRDS(params$pipeline_comparison_file)
} else {
...
```{r results='asis'}
old_data <- pc$dataset_column_values_comparison$old
...
if (!is.null(old_data) && nrow(old_data) > 0) {
  ...
  old_data %>%
    ...
    reactable(
      ...
      columns = list(
        `Column Name` = colDef(minWidth = 200),
        `Number Percentage` = colDef(
    ...
    cat("⚠️ Column name changes so unable to compare values")
}
...
```{r results='asis'}
new_data <- pc$dataset_column_values_comparison$new
