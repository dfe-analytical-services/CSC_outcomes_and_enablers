...
if (TRUE == FALSE) {
  ...
  x <- {
    test_sn(
      ...
      select_geo_breakdown = select_geo_breakdown,
      dimensional_filters = list(
        "cla_group" = "Distance between home and placement",
  ...
  x <- {
    test_sn(
      ...
      dimensional_filters = list(
        "cla_group" = "Placement",
        "characteristic" = "Foster placements"
      ),
      verbose = FALSE
    )
  }
  ...
  x[
    ...
    (cols_to_update) := eval(parse(
      text = "round(sdq_score_recd_x_score / sdq_score_recd, digits = 1)"
    ))
  ]
  x
  names(x)
  ...
  x[
    geographic_level == "Statistical neighbours (median)",
    (cols_to_update) := eval(parse(
      text = "round(100 * as.numeric(number) / total_number, digits = 0)"
    ))
  ]
  x
  names(x)
  ...
  if (calc_name != "") {
    sn_metrics[, eval(quote(calc_name)) := round(eval(aggregated_calc))]
  }
  sn_metrics
